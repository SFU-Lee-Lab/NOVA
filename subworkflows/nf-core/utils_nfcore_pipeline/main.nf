...
def workflowCitation() {
    ...
    temp_doi_ref += "  https://doi.org/${clean_doi}\n"
    }
...
//
def completionEmail(summary_params, email, email_on_fail, plaintext_email, out_dir, monochrome_logs=true, multiqc_report=null) {
    ...
    nextflow.extension.FilesEx.copyTo(output_hf.toPath(), "${out_dir}/PIPELINE_INFO/pipeline_report.html");
    output_hf.delete()

    // Write summary e-mail TXT to a file
    def output_tf = new File(workflow.launchDir.toString(), ".pipeline_report.txt")
    output_tf.withWriter { w -> w << email_txt }
    nextflow.extension.FilesEx.copyTo(output_tf.toPath(), "${out_dir}/PIPELINE_INFO/pipeline_report.txt");
    output_tf.delete()
}
